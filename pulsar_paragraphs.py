...
df = pd.read_csv('databasev6.csv',  header=None, sep='~', engine='python')
...
for i in range(len(psr_list)):
    ...
    if 'None' not in survey_func_str:
        if year_str == '':
            ...
            survey_str = ' as part of ' + '<a target="_top" href="' + 'https://astronomy.swin.edu.au/~mbailes/encyc/' + survey_name + '_plots.html' + '">' + survey_func_str + '</a>' + '.'
    else:
        survey_str = ''
    # ORBITAL PERIOD
