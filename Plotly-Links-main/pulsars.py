...
# Pulsar Page for the Swinburne Pulsar Encyclopedia of Radio Pulsar Astronomy !
...
# Load in list of available PSRs on MeerTime
...
html_intro_paragraph = """
 <p class="indent-all">
 For each pulsar we list its J2000 name (in RA order), its dispersion measure, and a nice description of its properties,
 plus a link to the survey that discovered it. If the pulsar is present in the <a target="_blank" href="https://pulsars.org.au">Swinburne Pulsar Portal</a> a link is provided
 so that you can see its mean pulse profile and timing history. Currently, there are over 25000 pulse profiles from 1544
 unique pulsars provided by the MeerTime large survey project on the MeerKAT telescope.
 <br>
"""
html_str_one = """
    <body>
    ...
    Radio pulsars are rotating neutron stars that emit a light-house like beam of radio emission
    that is detectable by radio telescopes. Here is a comprehensive list of all the pulsars
    in version """ + version + """ of the ATNF pulsar catalogue.
    </p>
"""

html_file = open('pulsars.html', 'w+')
html_start_str = '<html>\n' + ' <head>\n' + '  <title>\n' + 'Pulsars' + '  </title>' + ' </head>' + ' <body>\n'
html_end_str = '\n</body>\n' + '</style>\n</html>'
print(html_css_navbar_str + html_header_str + padding_str + html_str_one + html_intro_paragraph + '<hr>\n', file = html_file)
print(html_start_str + padding_str, file = html_file)
...
for i in range(len(psr_list)):
    ...
    if 'None' not in period_func_str:
        ...
        if 'None' not in dm_func_str:
            if str(psr_list[i]).strip() in psrs_available:
                period_str = 'PSR ' + '<a id=' + str(psr_list[i]).strip() + ' target="_blank" href="https://pulsars.org.au/fold/meertime/' + str(psr_list[i]).strip() + '">' + str(psr_list[i]).strip() + '</a>' + bname_str + ' is ' + period_func_str
            else:
                # print(str(psr_list[i]), str(i))
                period_str = '<a id=' + str(psr_list[i]).strip() + '>' + 'PSR ' + str(psr_list[i]) + bname_str + ' is ' + period_func_str
        else:
            if str(psr_list[i]).strip() in psrs_available:
                period_str = 'PSR ' + '<a id=' + str(psr_list[i]).strip() + ' target="_blank" href="https://pulsars.org.au/fold/meertime/' + str(psr_list[i]).strip() + '">' + str(psr_list[i]).strip() + '</a>' + bname_str + ' is ' + period_func_str + '.'
            else:
    ...
    if 'None' not in survey_func_str:
        if year_str == '':
            ...
            survey_str = ' as part of ' + '<a target="_blank" href="' + 'https://astronomy.swin.edu.au/~mbailes/encyc/' + survey_name + '_plots.html' + '">' + survey_func_str + '</a>' + '.'
    else:
        survey_str = ''
    # ORBITAL PERIOD
    ...
    end_str =  period_str + dm_str + s1400_str + dec_str + assoc_str + p1_func_str + pb_str + ecc_str + age_str + bsurf_str + vtrans_str + dist_str + minmass_str + year_str + survey_str + '<hr>' + '\n'
